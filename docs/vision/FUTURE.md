...
- [ ] **< 5% cloud LLM usage** (local models handle majority of tasks)
      USER NOTE: This is not the vision.  The vision is "right tool, for right job, for right scale" FREE TIER API access is the constraint, but that's not to say LOCAL handles majority of tasks.  LOCAL is just something we can always count on, but might not always be the right tool for the right job.  
